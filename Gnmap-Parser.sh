...
# Global Variables
parsedir="Parsed-Results_$(date +%Y%m%d_%H%M%S)"
...
#  clear
#  echo '============================================================================'
#  echo ' Gnmap-Parser.sh | [Version]: 4.0.0 | [Updated]: 10.14.2025'
#  echo '============================================================================'
#  echo
#}
...
func_parse(){
  ...
  PRNRULE01=$(grep " 9100/open/tcp" *.gnmap|cut -d" " -f2)
  PRNRULE02=$(grep " 515/open/tcp" *.gnmap|cut -d" " -f2)
  PRNRULE03=$(grep " 631/open/tcp" *.gnmap|cut -d" " -f2)
  echo ${PRNRULE01} ${PRNRULE02} ${PRNRULE03}|tr ' ' '\n'|${ipsorter} > ${hosttype}/Printers.txt

  #func_title
  echo '[*] Building Host-Type Domain Controllers List'
  DCRULE01=$(grep " 88/open/tcp" *.gnmap|grep -v "22/open/tcp"|cut -d" " -f2)
  DCRULE02=$(grep " 389/open/tcp" *.gnmap|grep -v "22/open/tcp"|cut -d" " -f2)
  DCRULE03=$(grep " 636/open/tcp" *.gnmap|grep -v "22/open/tcp"|cut -d" " -f2)
  DCRULE04=$(grep " 3268/open/tcp" *.gnmap|grep -v "22/open/tcp"|cut -d" " -f2)
  DCRULE05=$(grep " 3269/open/tcp" *.gnmap|grep -v "22/open/tcp"|cut -d" " -f2)
  echo ${DCRULE01} ${DCRULE02} ${DCRULE03} ${DCRULE04} ${DCRULE05}|tr ' ' '\n'|${ipsorter} > ${hosttype}/DomainControllers.txt
